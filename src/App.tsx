...
function App() {
  ...
  return <AppShell>{stage === 'home' && <div className="animate-enter-up relative flex min-h-[calc(100vh-4rem)] flex-col justify-between py-5"><div className='space-y-4 text-center'><StickerLabel>恋爱观察室 · 今日关系雷达扫描中</StickerLabel><h1 className='title-pop text-5xl font-black leading-[1.1] text-slate-800'>测测你和TA<br/>到底是什么关系</h1><SoftCard className='text-left'><div className='hero-chat'><span>你们到底啥关系？</span><span>问号爱心已就位 💗❓</span></div><p className='mt-3 text-sm text-slate-600'>20道轻松小题，测完就有“关系真名 + 双人海报”，专治嘴硬和暧昧。</p></SoftCard></div><div className='space-y-2'><GradientButton onClick={() => setStage('quiz')}>开始测试</GradientButton><p className='text-center text-xs text-slate-400'>答题免费，完整结果可1元解锁。</p></div></div>}

{stage==='quiz'&&<div className='animate-enter-up space-y-4 py-4'><SoftCard><ProgressBar current={currentQuestion+1} total={QUESTIONS.length}/><div className='mt-3 flex items-center justify-between'><p className='text-[11px] text-slate-400'>凭直觉选，关系雷达最吃第一反应。</p><button onClick={()=>currentQuestion>0&&setCurrentQuestion((p)=>p-1)} disabled={currentQuestion===0} className='rounded-xl border border-violet-200 bg-white px-3 py-1.5 text-xs font-semibold text-violet-500 disabled:opacity-40'>上一题</button></div></SoftCard><SoftCard className='animate-enter-up'><p className='text-xs font-semibold text-slate-400'>Relationship Question</p><h2 className='mt-1 text-[1.4rem] font-black text-slate-800'>{QUESTIONS[currentQuestion].title}</h2></SoftCard><div className='space-y-3'>{QUESTIONS[currentQuestion].options.map((option,idx)=>{const isSelected=selectedOptionId===option.id;return <ChoiceCard key={option.id} selected={isSelected} onClick={()=>onAnswer(option.id)}><div className='flex items-center gap-2'><span>{optionStickers[idx%4]}</span><span className='font-semibold text-violet-600'>{option.label}.</span>{isSelected&&<span className='rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-semibold text-violet-600'>已锁定</span>}</div><p className='mt-1 pl-6 text-sm text-slate-600'>{option.text}</p></ChoiceCard>;})}</div></div>}

{stage==='loading'&&<div className='flex min-h-[calc(100vh-5rem)] items-center'><SoftCard className='w-full text-center'><p className='text-xs font-semibold uppercase tracking-[0.2em] text-violet-400'>Relationship Scanner</p><div className='scanner mt-4 mx-auto' /><h2 className='mt-5 text-xl font-black text-slate-800'>关系扫描仪启动中</h2><p className='mt-2 text-sm text-slate-500'>{loadingSteps[loadingTextIndex]}</p></SoftCard></div>}
